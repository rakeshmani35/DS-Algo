                    	 ...
                    	 while(left>=0 && right<grid[i].length() && grid[i].charAt(left)==grid[i].charAt(right) && grid[i].charAt(left) == grid[i].charAt(j)
                       	  ...
                       	  if(h2Count > maxH) {
                       		maxH = h2Count;
								 ...
								 * else if(maxH >= h2Count){ h2Count = maxH; }
								 */
                       	
                         }
                         move2=0;
                         while(up>=0 && down<grid[j].length()-1 && grid[up].charAt(j)==grid[down].charAt(j) && grid[up].charAt(j) == grid[i].charAt(j)
                        		 && grid[down].charAt(j) == grid[i].charAt(j) && move2<=move1 && h2Count!=v2Count) {
                       	 ...
                       	 if(v2Count > maxV) {
                       		maxV = v2Count;
								 ...
								 * else if(maxV >= h2Count){ v2Count = maxV; }
								 */
                       	
                         }
                         secondValidPlus = 1;
						/*
						 * if(h2Count > v2Count) { maxH = v2Count; }else if (h2Count < v2Count) { maxV =
						 * h2Count; }else if (h2Count == v2Count){ maxH = maxV = h2Count; }
						 */
					}
